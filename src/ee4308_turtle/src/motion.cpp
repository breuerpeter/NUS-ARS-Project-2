...
int main(int argc, char **argv)
{
    ...
    //data file to log velocity of turtle bot
    std::ofstream turtle_vel_file;
    // replace with local file directory
    // file path as follows "/home/<user>/<project folder>/turtle_vel_data.txt"
    turtle_vel_file.open("/home/khin/project2/data.txt");
    if (turtle_vel_file.fail()) ROS_INFO("T Vel: File Open Failed");
    else ROS_INFO("T Vel: File Open Success");
    ...
    { // subscribes to odom topic --> is the exact simulated position in gazebo; when used in real life, is derived from wheel encoders (no imu).
        ...
        while (ros::ok() && nh.param("run", true))
        {
            ...
            // log velocity of turtle bot
            turtle_vel_file << prev_time << "\t" << msg_odom.twist.twist.linear.x << std::endl;
