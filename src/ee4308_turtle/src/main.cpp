...
int main(int argc, char **argv)
{
    ...
    int spline_type;
    if (!nh.param("spline_type", spline_type, 0))
        ROS_WARN(" TMAIN : Param spline_type not found, set to 0 (none)");
     bool use_cubic;
    if (!nh.param("use_cubic", use_cubic, true))
        ROS_WARN(" TMAIN : Param use_cubic not found, set to true");
     bool use_euc;
    if (!nh.param("use_cubic", use_euc, true))
        ROS_WARN(" TMAIN : Param use_euc not found, set to true");
        ...
        if (replan)
        {
            if (grid.get_cell(pos_rbt) && grid.get_cell(pos_goal))
            {
                    ...
                    for (int m = 1; m < post_process_path.size(); ++m)
                {
		            ...
		            if (spline_type == 0)
                            traj = generate_trajectory_no_spline(turn_pt_next, turn_pt_cur, average_speed, target_dt, grid);
                    else if (spline_type == 3)
                            traj = generate_trajectory_spline(turn_pt_next, turn_pt_cur, average_speed, target_dt, grid, use_cubic);
                    ...
                    // publish trajectory to trajectory topic
